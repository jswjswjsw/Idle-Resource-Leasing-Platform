...
router.get('/login', async (req: Request, res: Response) => {
  try {
    ...
    // 生成微信OAuth授权URL
    const authUrl = wechatOAuthService.generateAuthUrl(state);
...
router.post('/callback', async (req: Request, res: Response) => {
  try {
    ...
    // 处理微信OAuth登录
    const loginResult = await wechatOAuthService.handleOAuthLogin(code as string);
...
router.post('/mobile', async (req: Request, res: Response) => {
  try {
    ...
    // 处理微信OAuth登录
    const loginResult = await wechatOAuthService.handleOAuthLogin(code);
 ...
 * @access Private
 */
router.delete('/unbind', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.userId; // 从认证中间件获取用户ID
...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    const response: ApiResponse<{ isBound: boolean; displayName?: string }> = {
      success: true,
      message: '获取微信绑定状态成功',
      data: { 
