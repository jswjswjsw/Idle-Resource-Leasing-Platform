...
router.post('/create',
  ...
  validate({
    body: Joi.object({
      ...
      amount: Joi.number().required().min(1).max(100000000).messages({
      ...
      method: Joi.string().optional().valid(...Object.values(PaymentMethod)).messages({
        'any.only': '不支持的支付方式'
      }),
      returnUrl: Joi.string().optional().uri().messages({
        'string.uri': '返回URL格式不正确'
      }),
      notifyUrl: Joi.string().optional().uri().messages({
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await paymentService.createPayment(orderInfo, method);
...
router.get('/status/:paymentId',
  ...
  asyncHandler(async (req, res) => {
    ...
    const status = await paymentService.queryPaymentStatus(paymentId);
...
router.post('/refund',
  ...
  validate({
    body: Joi.object({
      ...
      refundAmount: Joi.number().required().min(1).messages({
        'number.base': '退款金额必须是数字',
        'number.min': '退款金额必须大于0',
        'any.required': '退款金额是必需的'
      }),
      reason: Joi.string().required().min(1).max(200).messages({
