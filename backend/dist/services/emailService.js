...
function initEmailService() {
    try {
        ...
        if (!emailConfig.host || !emailConfig.auth.user || !emailConfig.auth.pass) {
            logger_1.winstonLogger.warn('邮件服务配置不完整，邮件功能将被禁用');
            return null;
        }
        const transporter = nodemailer_1.default.createTransporter(emailConfig);
        // 验证SMTP连接
        transporter.verify((error, success) => {
            if (error) {
                logger_1.winstonLogger.error('邮件服务连接失败', { error: error.message });
            }
            else {
                logger_1.winstonLogger.info('邮件服务连接成功');
            }
        });
        return transporter;
    }
    catch (error) {
        logger_1.winstonLogger.error('邮件服务初始化失败', { error });
        return null;
    }
}
// 初始化邮件服务
...
async function sendEmail(options) {
    try {
        if (!transporter) {
            logger_1.winstonLogger.warn('邮件服务未初始化，无法发送邮件');
            return false;
        }
        ...
        logger_1.winstonLogger.info('邮件发送成功', {
            to: options.to,
            subject,
        ...
        logger_1.winstonLogger.error('邮件发送失败', {
            error: error.message,
            to: options.to,
 ...
 * 发送验证码邮件
 */
async function sendVerificationEmail(email, username, code, expireMinutes = 10) {
    return sendEmail({
        to: email,
 ...
 * 发送密码重置邮件
 */
async function sendPasswordResetEmail(email, username, resetUrl, expireMinutes = 60) {
    return sendEmail({
        to: email,
 ...
 * 发送登录通知邮件
 */
async function sendLoginNotificationEmail(email, username, loginTime, ip, location) {
    return sendEmail({
        to: email,
 ...
 * 发送订单通知邮件
 */
async function sendOrderNotificationEmail(email, username, orderNumber, resourceTitle, totalAmount, action) {
    return sendEmail({
        to: email,
