...
const database_1 = require("./config/database"); // 复用统一的Prisma客户端，避免多实例与类型问题
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const isValidPassword = user.password ? await bcryptjs_1.default.compare(password, user.password) : false;
        if (!isValidPassword) {
            return res.status(401).json({
                success: false,
                message: '用户不存在或密码错误'
...
app.put('/api/resources/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const updatedResource = await database_1.prisma.resource.update({
            where: { id: req.params.id },
            data: {
                title,
                description,
