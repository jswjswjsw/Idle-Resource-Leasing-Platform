...
router.get('/search',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (priceUnit) filter.priceUnit = priceUnit as string;
...
router.delete('/:id/images/:imageIndex',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // 删除指定索引的图片
      const updatedImages = resource.images.filter((_: any, idx: number) => idx !== index);
...
router.get('/nearby',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const radius = parseFloat(req.query.radius as string) || 10; // 默认10公里
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 20;
