...
export class WechatOAuthService {
   ...
   * @returns 登录结果
   */
  async handleOAuthLogin(code: string): Promise<OAuthLoginResult> {
    try {
      // 1. 获取访问令牌
   ...
   * @returns 唯一用户名
   */
  private async generateUniqueUsername(nickname: string, tx: Prisma.TransactionClient = prisma): Promise<string> {
    // 清理昵称，只保留字母数字和中文
