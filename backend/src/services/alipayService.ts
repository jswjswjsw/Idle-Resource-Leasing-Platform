...
class AlipayService {
   ...
   * 创建支付宝支付订单
   */
  async createOrder(orderId: string, amount: number, subject: string, returnUrl?: string, notifyUrl?: string) {
    try {
   ...
   * 创建手机网站支付订单
   */
  async createWapOrder(orderId: string, amount: number, subject: string, returnUrl?: string, notifyUrl?: string) {
    try {
      const order = await prisma.order.findUnique({
        where: { id: orderId },
        include: { resource: true, renter: true }
  ...
  async queryOrderStatus(orderId: string) {
    try {
      ...
      if (result.code === '10000') {
        ...
        throw new AppError(result.sub_msg || '查询失败', 400);
      }
    } catch (error) {
  ...
  async handleNotify(data: any) {
    try {
      ...
      if (tradeStatus === 'TRADE_SUCCESS' || tradeStatus === 'TRADE_FINISHED') {
        await paymentBaseService.handleSuccessfulPayment(orderId, tradeNo);
      } else if (tradeStatus === 'TRADE_CLOSED') {
        await paymentBaseService.handleFailedPayment(orderId, tradeNo);
      }
  ...
  async refund(orderId: string, refundAmount: number, reason: string) {
    try {
      ...
      if (result.code === '10000') {
        ...
        throw new AppError(result.sub_msg || '退款失败', 400);
      }
    } catch (error) {
